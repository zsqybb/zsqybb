...
def api_champion_build(champ_id):
    ...
    # 如果指定了分路，且该分路有特定数据，则使用分路数据
    if position and build_data.get('positions'):
        pos_data = next((p for p in build_data['positions'] if p['name'] == position), None)
        if pos_data:
            result = dict(build_data)
            # 用分路统计数据覆盖通用统计
            for key in ['win_rate', 'pick_rate', 'ban_rate', 'kda', 'tier']:
                if key in pos_data:
                    result[key] = pos_data[key]
            # 如果该分路有特定的builds/runes/skills，覆盖通用数据
            if 'builds' in pos_data:
                result['builds'] = pos_data['builds']
            if 'runes' in pos_data:
                result['runes'] = pos_data['runes']
            if 'skills' in pos_data:
                result['skills'] = pos_data['skills']

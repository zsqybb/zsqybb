...
// ==================== 图标映射 ====================
let _iconMaps = null;  // { items: {名: id}, runes: {名: icon}, rune_trees: {名: icon} }
...
function getItemIcon(itemName) {
    if (!_iconMaps) return '';
    const id = _iconMaps.items[itemName];
    return id ? `/static/img/item/${id}.png` : '';
}

function getRuneIcon(runeName) {
    if (!_iconMaps) return '';
    const icon = _iconMaps.runes[runeName];
...
let _currentChampPositions = [];   // 当前英雄各分路数据
let _currentPositionName = '';       // 当前选中分路名(英文)
...
async function openChampionDetail(champId) {
    ...
    _currentChampId = champId;  // 存储当前英雄ID，分路切换时需要
    ...
    try {
        ...
        if (respBuild.ok) {
            try {
                ...
                if (buildData.success) {
                    ...
                    // 如果positions有数据但main_position为空，取第一个
        ...
        if (build && build.builds) {
            ...
            html += `<div class="build-section" id="buildSection">`;

            if (b.starts) {
                html += `<div class="build-group"><div class="build-label">起始装备</div><div class="build-items">`;
                b.starts.forEach(item => {
                    const icon = getItemIcon(item);
                    html += `<span class="build-item">${icon ? `<img src="${icon}" class="item-icon" onerror="this.style.display='none'">` : ''}${item}</span>`;
                });
                html += `</div></div>`;
            }
            ...
            if (b.core) {
                ...
                b.core.forEach(item => {
                    const icon = getItemIcon(item);
                    html += `<span class="build-item core">${icon ? `<img src="${icon}" class="item-icon" onerror="this.style.display='none'">` : ''}${item}</span>`;
                });
                html += `</div></div>`;
            }
        ...
        if (build && build.runes) {
            ...
            html += `<div class="runes-section" id="runesSection">`;
            // 主系
        ...
        if (build && build.skills) {
            ...
            html += `<div class="skills-order" id="skillsSection">推荐加点顺序: <span class="skills-order-text">${build.skills}</span></div>`;
        }
...
function updateBuildSection(build) {
    ...
    if (!container || !build || !build.builds) return;
    const b = build.builds;
    let html = '';
    if (b.starts) {
        html += `<div class="build-group"><div class="build-label">起始装备</div><div class="build-items">`;
...
function updateRunesSection(build) {
    ...
    if (!container || !build || !build.runes) return;
    const r = build.runes;
    let html = '';
    const primaryIcon = getRuneTreeIcon(r.primary || '精密');
    html += `<div class="rune-tree"><div class="rune-tree-name">${primaryIcon ? `<img src="${primaryIcon}" class="rune-tree-icon" onerror="this.style.display='none'">` : ''}${r.primary || '精密'}</div>`;
...
function updateSkillsSection(build) {
    ...
    if (!container || !build || !build.skills) return;
    container.innerHTML = `推荐加点顺序: <span class="skills-order-text">${build.skills}</span>`;
}
...
function updateRankingCards(posData) {
    ...
    let html = '';
    if (winRate) {
        html += `<div class="ranking-card"><div class="ranking-value ${parseFloat(winRate) >= 52 ? 'good' : parseFloat(winRate) < 48 ? 'bad' : ''}">${winRate}%</div><div class="ranking-label">胜率</div></div>`;
    }
    if (pickRate) {
        html += `<div class="ranking-card"><div class="ranking-value">${pickRate}%</div><div class="ranking-label">选取率</div></div>`;
    }
    if (banRate) {
        html += `<div class="ranking-card"><div class="ranking-value ${parseFloat(banRate) >= 10 ? 'hot' : ''}">${banRate}%</div><div class="ranking-label">禁用率</div></div>`;
    }
    if (kda) {
        html += `<div class="ranking-card"><div class="ranking-value">${kda}</div><div class="ranking-label">KDA</div></div>`;
    }
    ...
    if (titleEl && titleEl.classList.contains('section-title')) {
        const label = _currentChampPositions.length > 1 ? ` (${positionName})` : '';
        titleEl.innerHTML = `<i class="fas fa-chart-bar"></i> 韩服排位数据${label}`;
    }
}

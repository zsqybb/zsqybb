...
async function openChampionDetail(champId) {
    ...
    try {
        ...
        const imgSrc = `/static/img/champion/${champ.image.full}`;
        const passiveImg = `/static/img/passive/${champ.passive.image.full}`;

        // 难度星星
        const diff = (build && build.difficulty) || 2;
        ...
        <div class="champion-detail-header">
            <img src="${imgSrc}" onerror="this.src='/static/img/champion/Akali.png'" alt="${champ.name}">
            <div>
        ...
        html += `
            <div class="ability-item">
                ...
                <div>
                    <div class="ability-name">${champ.passive.name} <span style="color:var(--gold)">[被动]</span></div>
                    <div style="font-size:11px;color:var(--text-muted)">${champ.passive.description || ''}</div>
                </div>
            </div>`;

        const spellKeys = ['Q', 'W', 'E', 'R'];
        champ.spells.forEach((spell, i) => {
            const spellImg = `/static/img/spell/${spell.image.full}`;
            ...
            <div class="ability-item">
                ...
                <div>
                    <div class="ability-name">${spell.name} <span style="color:var(--blue)">[${spellKeys[i]}]</span></div>
